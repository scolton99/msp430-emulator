...
TEST(ProgTest, ELFFillMissing) {
  ...
  EXPECT_THROW_MSG(BIN::ELFObject::fill(&elf, "data/non-existent.out"),
                   std::runtime_error,
                   "File does not exist");
}
...
TEST(ProgTest, ELFFillSimple) {
  BIN::ELFObject elf;
  BIN::ELFObject::fill(&elf, "data/simple.out");
...
TEST(ProgTest, ELFFillComplex) {
  BIN::ELFObject elf;
  BIN::ELFObject::fill(&elf, "data/complex.out");
...
TEST(ProgTest, ELFFillMalformed) {
  ...
  EXPECT_THROW_MSG(BIN::ELFObject::fill(&elf, "data/malformed.out"),
                   std::runtime_error,
